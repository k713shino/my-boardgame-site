...
export function SharedRosterById({ id }: { id: string }) {
  const [roster, setRoster] = useState<SharedData | null>(null);
  const [error, setError] = useState(false);

  useEffect(() => {
    fetch(`/api/wh40k/share/${id}`)
      .then((r) => {
        if (!r.ok) throw new Error("not_found");
        return r.json();
      })
      .then(({ data }: { data: string }) => {
        const parsed: SharedData = JSON.parse(data);
        setRoster(parsed);
      })
      .catch(() => setError(true));
  }, [id]);
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-[0.68rem] capitalize text-muted">
              {roster.factionName ?? roster.faction} · {roster.pointsLimit}pt
